...
export async function fetchEnv() {
  ...
  json.data.forEach((item) => {
    envVariables[toCamelCase(item.Key)] = item.Value;
  });
  window.envVariables = envVariables;
  return window.envVariables;
}
...
export async function decorateIcons(element) {
  ...
  icons.forEach((span) => {
    ...
    if (ICONS_CACHE[iconName].styled) {
      ...
      parent.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" title="svg ${label}" role="img" ><use href="#icons-sprite-${iconName}" role="figure" title="${label}"/></svg>`;
    }
  });
}
